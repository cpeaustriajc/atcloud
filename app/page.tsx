...
export default async function Home() {
    ...
    const getTimezone = (timezone:string) => {
        const locationPart = timezone.includes('/')
        ? timezone.split('/').pop() || timezone
        : timezone;

        return locationPart.replace(/_/g, ' ');
    }
        ...
        <>
            <div className="grid place-items-center place-content-center h-full gap-2">
                ...
                <h1 className="text-5xl font-bold">
                    {toCelsius(data.current.temp)}&deg;C
                </h1>
                <p className="text-2xl">{data.current.weather[0].main}</p>
                <p className="text-2xl font-bold">{getTimezone(data.timezone)}</p>
                <p className="text-xs">
                    Feels Like: {toCelsius(data.current.feels_like)}&deg;C |
                    Humidity: {data.current.humidity}{' '}
                </p>
                <p className="text-xs">
                    Wind Direction: {getWindDirection(data.wind_deg)} | Wind
                    Speed: {data.wind_speed} km/h{' '}
                </p>
            </div>
            <div className="h-[5%] flex justify-center items-center flex-col">
                <p className="text-sm">
                    App made with Next.js and Edge Functions
                </p>
                <p className="text-sm">
                    Made by:{' '}
                    <a
